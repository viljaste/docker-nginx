class nginx::ssl {
  ...
  exec { "openssl req -x509 -new -nodes -key /nginx/ssl/private/nginxCA.key -days 365 -subj /C=/ST=/L=/O=/CN=nginx -out /nginx/ssl/certs/nginxCA.crt":
    timeout => 0,
    path => ['/usr/bin'],
  ...
  exec { 'openssl genrsa -out /nginx/ssl/private/nginx.key 4096':
    timeout => 0,
    path => ['/usr/bin'],
    require => Exec["openssl req -x509 -new -nodes -key /nginx/ssl/private/nginxCA.key -days 365 -subj /C=/ST=/L=/O=/CN=nginx -out /nginx/ssl/certs/nginxCA.crt"]
  }
